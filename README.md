...
본 설계는 CNN 가속기에 대해 Adder Tree Pipelining과 Ternary MAC을 단계적으로 적용하면서, 정확도·면적·타이밍·전력 간의 트레이드오프를 체계적으로 검증하였다. 중간 설계는 기존 구조 대비 면적 증가라는 비용을 지불하는 대신, 타이밍 위반을 해소하고 정확도와 에너지 효율을 동시에 끌어올린 단계이다. 최종 설계에서는 여기에 Ternary MAC을 도입해 곱셈기를 부호 가산 구조로 대체함으로써, 전력과 면적을 다시 줄이고 TOPS/W를 약 2.2배 추가로 향상시키는 대신, 정확도 93%라는 일정 수준의 성능 저하를 수용하였다.

종합하면, 제안한 최종 구조는 정확도 3%p 감소와 면적 약 26% 증가를 감수하는 대가로, 기존 Reference 설계 대비 전력 93% 감소, TOPS/W 약 47배 향상을 달성한 에너지 효율 중심의 하드웨어 아키텍처이다. 이는 제한된 전력 예산 하에서 고속 CNN 추론이 요구되는 엣지·임베디드 환경에서, 파이프라이닝과 저비트 연산(ternary) 결합이 매우 효과적인 설계 전략이 될 수 있음을 보여준다.
